...
; @done  Disassembled to instructions (byte-exact), shared-engine calls
; symbolised, split one-file-per-routine (STARThex-ENDhex__name). Every routine
; named + hand-polished by behaviour (6 dispatch handlers, the cell/maze helpers,
; the special-location event handlers, the 3D-view renderer hierarchy, movement);
; 0 unnamed l_XXXX left. Bootstrapped by tools/regen_level03_handlers.sh
; (disasm+name+symbolize) then split by tools/split_level03.py; now hand-maintained.
; Analogous to level_02's $D1D8-$DC51 handler region (all @done).
sewers_handlers:
...
; $DF0D-$F3FF - the 45 labelled 3D-view sprite bitmaps (source)
	...
	include "levels/level_03/code/F1A9-F3FF__sprite_44.asm"	; sprite_44 (level-specific backdrop)
...
; Flat 2-byte {SO_NO,WE_EA} cell entries partitioned by special_dispatch_table:
; for event N it scans `count` pairs from special_loc_list+`offset` (both from the
; record); a pair matching the party cell fires that event. $FF pairs = empty
; slots. Same format as level_02's (verified @done there). Kept as .bin data.
special_loc_list:
